...
import kotlinx.android.synthetic.main.activity_main.main_tb
import kotlinx.android.synthetic.main.activity_main.nav_host_fg
...
class MainActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    mActivityLiveData.observe(this, Observer { value ->
      Log.info("LiveData 收到当前的状态为 $value")
    })
...
//        }
    ...
    val navHostFragment =
      supportFragmentManager.findFragmentById(R.id.nav_host_fg) as NavHostFragment
    val navGraph =
      navHostFragment.navController.navInflater.inflate(R.navigation.nav_main_fragment_page)

    val navArgument = NavArgument.Builder()
        .setDefaultValue("默认数据")
        .build()
...
//        NavigationUI.setupWithNavController(main_menu_bnv, navController)
  ...
  override fun onDestroy() {
    Log.info("${javaClass.simpleName} onDestroy")
